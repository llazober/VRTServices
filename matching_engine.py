...
def match_gl_account(
    ...
    Returns (account_number, account_name, confidence_score, matched_rule_description)
    
    matched_rule_description is the 'description' field from the matched ClientTransactionHistory rule.
    It is empty string ("") when no explicit rule match is found (heuristic or fallback paths).
    This value is used to populate the Reference field in the output CSV when checknumber is absent.
    """
    ...
    for rule in history_rules:
        ...
        rule_desc = str(rule.get("description", "") or "").strip()
        ...
        if clean_desc == pattern or raw_upper == pattern:
            return (acct_num, acct_name, 1.0, rule_desc)
        ...
        if len(pattern) >= 2:
            if re.search(r'\b' + re.escape(pattern) + r'\b', raw_upper) or re.search(r'\b' + re.escape(pattern) + r'\b', clean_desc):
                return (acct_num, acct_name, 0.95, rule_desc)
    ...
    if highest_ratio >= 0.70 and best_match:
        ...
        rule_desc = str(best_match.get("description", "") or "").strip()
        return (acct_num, acct_name, round(highest_ratio, 2), rule_desc)
        
    # Stage 3.5: Intelligent Banking & Vendor Heuristic Classification
    # NOTE: These heuristics do NOT come from a stored rule, so matched_rule_description = ""
    if is_deposit:
